...
Each channel attempted requires its own `run_decisions[]` entry whose `decision` field begins with `channel_<N>_` (where N is 1–4, matching the channel number below). Channel 1's per-platform sub-attempts (Legistar, PrimeGov, BoardDocs, etc.) go INSIDE the single `channel_1_*` entry's `reason` field — do NOT emit a separate `run_decisions[]` entry per sub-platform, that would inflate the count without exhausting the other 3 channels. Keep this `reason` terse — summarize the platforms probed (e.g. `"Legistar/PrimeGov/BoardDocs: no FY26 packet"`) rather than transcribing each attempt; every `reason` is capped at 280 characters. The deterministic validator (`/workspace/qa_checks.py`) extracts the `channel_<N>_` prefix from each decision and rejects any near-term `awaiting_agenda` / `no_meeting_found` artifact that doesn't show all 4 distinct channel numbers (beyond-cutoff meetings short-circuit after channel 1 per the publish-lag early-exit above). Stop early ONLY when you find packet content for the target meeting.
...
Channels 1–4 are the high-yield discovery set (primary platforms, city schedule page, deterministic PDF mirror, CDN web search). Lower-yield channels (local news re-hosting, the clerk/records page, and the past-meeting URL probe) were retired in the WS5 cost pass — they produced roughly 1% of successful packet finds and are not worth the per-run cost. If local-news coverage happens to surface a packet PDF while you are verifying the meeting date above, you may still fetch it; just attribute it under the relevant channel 1–4 entry.
