...
Two phases:
  ...
  2. Deterministic QA checks the schema cannot express:
       ...
       - awaiting_agenda / no_meeting_found: all 4 high-yield discovery channels attempted (channel_<N>_ prefixes)
...
def _select_branch_schema(artifact: dict, schema: dict) -> dict | None:
    """Pick the applicable `oneOf` branch by matching briefing_status against
    each branch's `properties.briefing_status.enum`.

    Returns a self-contained sub-schema (the matched branch with the parent's
    `definitions` carried over so `$ref`s still resolve) when EXACTLY ONE branch
    matches. Returns None — meaning "fall back to validating the full schema" —
    when the schema isn't a oneOf, briefing_status is missing, or zero/multiple
    branches match. The fallback preserves the original coverage so we never
    lose validation.
    """
    ...
    for branch in branches:
        ...
        if isinstance(enum, list) and status in enum:
            matches.append(branch)

    if len(matches) != 1:
        return None

    # Carry the parent's shared keys (definitions, $schema) onto the branch so
    # internal $refs like "#/definitions/item" still resolve when we validate
    # against the branch alone.
    branch_schema = dict(matches[0])
    for shared_key in ("definitions", "$schema"):
        if shared_key in schema and shared_key not in branch_schema:
            branch_schema[shared_key] = schema[shared_key]
...
def validate_schema(artifact: dict, schema: dict) -> list[str]:
    ...
    Branch-aware: when the schema is a two-branch `oneOf` (Full / Placeholder)
    and the artifact's briefing_status unambiguously selects one branch, we
    validate against ONLY that branch so errors are scoped and actionable
    instead of the merged "doesn't match Full AND doesn't match Placeholder"
    noise the raw oneOf emits. When the branch is ambiguous or missing, we fall
    back to validating against the full schema (original behavior) to preserve
    coverage.
    """
    effective_schema = _select_branch_schema(artifact, schema) or schema
...
# `no_meeting_on_target_date` reason instead of running packet discovery.
# Packet discovery only applies when a target meeting was confirmed but the
# packet may or may not exist yet (the `awaiting_agenda` path).
...
def check_awaiting_agenda_discovery_depth(artifact: dict, findings: list[Finding]) -> None:
    ...
    (the stale-schedule signal path under #58's verify-not-discover flow) are
    exempt entirely: the agent never reached packet discovery because the
    caller-supplied target meeting did not exist on the platform.
