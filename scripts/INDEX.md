...
| `python/meeting_briefing_output_schema.json` | JSON Schema (Draft-07) for the v2 meeting_briefing artifact. Staged for inlining into `experiments/meeting_briefing/manifest.json` at port time | books/run-meeting-briefing.md |
| `python/validate_meeting_briefing.py` | Validate a v2 meeting_briefing artifact against the schema AND run deterministic QA checks (cross-reference integrity, required_data_points coverage, tier_reason consistency, source-extract presence). No LLM calls. Will become `experiments/meeting_briefing/validate_output.py` at port time | books/run-meeting-briefing.md |
