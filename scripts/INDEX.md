...
| `python/fixtures/qa_test_packet/build_variants.py` | Regenerate the qa-spine golden-fixture layout (unit_check_packet.json + thin integration variants) from the clean baseline. Idempotent; synthetic content only. | scripts/python/test_qa_spine_unit_checks.py, test_qa_spine_integration_verdict.py |
| `python/test_qa_spine_unit_checks.py`, `python/test_qa_spine_integration_verdict.py` | Meta-tests for the QA spine: unit file asserts each deterministic check's result on a targeted case in the golden fixture (verdict-independent); integration file asserts whole-artifact release_verdict routing. Run with `uv run pytest`. | Golden fixture at `python/fixtures/qa_test_packet/` |
