...
| `python/qa_validate.py` | Validate a single LLM-generated artifact JSON: 12 deterministic checks + Phase 1 LLM triage + Phase 2 adversarial escalation → release_verdict (ok/warn/block) + per-claim verdicts in qa_bundle.json. Product-agnostic; product-specific rules live in a `<product>_product_spec.json`. Shipped to Fargate as a manifest attachment by the meeting_briefing experiment. | experiments/meeting_briefing (manifest attachment) |
