...
| `python/verify_urls.py` | Verify a list of URLs return HTTP 200 (HEAD first, fallback to GET on 403/405/501). Follows redirects, returns `{url, status, final_url, ok, error?}` per row as JSON. Reads URLs from argv or stdin. | books/find-opposition-research.md |
