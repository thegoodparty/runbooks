...
Usage:  uv run scripts/python/rubric_verdict.py experiment-evals/meeting_briefing/rubric_scores.tsv
"""
import sys
import statistics
...
def load(path):
    ...
    with open(path) as f:
        for line in f:
            ...
            if len(parts) < 4:
                continue
            uuid, batch, a, b = parts[0], parts[1], parts[2].strip(), parts[3].strip()
