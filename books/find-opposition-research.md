...
**Fan-out now ports to Fargate.** The harness gained native subagent fan-out, gated by the manifest field `runtime.max_parallel_subagents` (0 = off; cap 20). Set it (e.g. 6) and the harness wires a `researcher` subagent the parent dispatches concurrently via the `Agent` tool — one research unit per opponent — inheriting the parent's tools, model, permission mode, and broker scope. Keep step 4's sequential loop as the documented graceful-degrade fallback. The per-opponent return-contract JSON is still what the experiment's `output_schema` validates against — keep the runbook's contract and the manifest schema in sync.

**URL verification: use the cheap rung first.** `verify_urls.py` (local `requests`, HEAD/GET) does not port — the Fargate runner is quarantined. The web-access ladder there is **WebSearch (discover) -> `pmf_runtime.http.head(url)` (verify status, non-browser, cheap) -> `pmf_runtime.http.get(url)` (Chromium render, LAST RESORT)**. Verify citations with `http.head` (`r["status"] == 200`, plain dict — not `requests.Response`); escalate to `http.get` only when `head` returns 403/405 on a real site (Cloudflare bot-block the browser defeats) or you must read the page body. Re-rendering every URL with `http.get` is what made the first port time out — don't.
