...
- `structured_validators` — per-`claim_type` opt-in `{kind, rounding_tolerance}`. The `numeric_review_flag` check uses these to DETECT high-stakes figures: for a high-weight claim it extracts every literal of `kind` (`money`, `date`, `name`, `vote_count`, `legal_citation`, `percentage`) from `claim_text` and, if any are present, flags the claim so the triage judge must verify the figure against the cited extracts. Detection only — it never matches values and never blocks (route `diagnostic`); a wrong figure blocks only via the judge verdict. `rounding_tolerance` is unused by detection (kept for config compatibility).
