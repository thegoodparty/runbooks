...
| proc | smoke test, smoke, run experiment, dispatch experiment, verify experiment, test experiment in dev, run all experiments, experiment health check, pmf smoke | books/pmf-smoke.md | Dispatch a live smoke run of each PMF experiment in dev (SQS → Fargate) and verify it produces a valid artifact. Where to find experiments + prior runs, how to get valid params, dispatch wire format, log tailing, artifact validation. Notes that a smoke tests the DEPLOYED dev image, not your local branch. |
