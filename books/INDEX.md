...
| proc | release, production deploy, ship, qa to master, product-releases, release notes, deploy to prod | commands/release.md | Merge the pending `qa → master` PRs opened by `/release-prep` across configured repos, wait 5 minutes for deploy, then print a `#product-releases` message listing every released ENG-XXXX ticket with its ClickUp title (plus untagged fallbacks). Also `/release`. |
