...
| proc | opposition research, opponents, who is running, candidates in race, opposition, opposition section, campaign plan opposition | books/find-opposition-research.md | Produce the Opposition Research of a campaign plan from the injected election-api context (gp-api provides it; nothing calls election-api): seed opponents from the general + primary rosters, add late filers via web search, verify every cited URL returns 200, output structured JSON `{opponents:[...]}`. Source runbook for the `opposition_research` PMF experiment. |
...
| proc | opportunities, challenges, strategic landscape, campaign plan opportunities, campaign plan challenges, strengths and risks, structural advantages | books/find-opportunities-and-challenges.md | Produce the Opportunities and Challenges of a campaign plan: up to 3 structural advantages + up to 3 structural risks derived from the injected race numbers, each cited with URLs verified 200. Structured JSON output. Source runbook for the `opportunities_and_challenges` PMF experiment; ported from gp-api's strategic-landscape prompts. |
...
| proc | cost analysis, experiment cost, agent cost, runtime, success rate, per-step cost, where does spend go, average cost of job, token usage, cache_read, conversation.jsonl, session.jsonl, gp-agent-artifacts, artifact analysis | books/analyze-experiment-costs.md | Analyze cost, runtime, and success rate of PMF agent experiments from S3 artifact + log data. Covers the four common questions (avg cost, avg runtime, where spend happens, success rate), data inventory (S3 layout + file formats), reusable Python parsing utilities, Anthropic pricing reference, and pitfalls (sync timing, status field naming, task-tracking inconsistency, runner-reported vs token-derived cost). |
